...
function generateProductInfoHTML(data) {
    ...
    if (data.productType) {
        infoItems.push(`<tr><th></th><td>${data.productType}</td></tr>`);
    }
    if (data.origin) {
...
async function aiAssist(fieldType) {
    ...
    try {
        ...
        switch(fieldType) {
            ...
            case 'metaTitle':
                prompt = `"${formData.productName}"  SEO    60  .`;
                targetField = 'metaTitle';
                break;

            case 'metaDesc':
                prompt = `"${formData.productName}"  SEO    160  .`;
                targetField = 'metaDescription';
                break;

            case 'keywords':
                prompt = `"${formData.productName}"     10 .  .`;
...
function updatePreview() {
    ...
    const previewHTML = `
        <div style="padding: 20px; font-family: 'Pretendard', sans-serif;">
            <h2 style="color: #1e40af; margin-bottom: 20px;">${formData.productName || ' '}</h2>
...
async function generateDetailPage() {
    ...
    try {
        ...
        // 5. HTML  ( generator.js generateHTML  )
        let htmlContent;
