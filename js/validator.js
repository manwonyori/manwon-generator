...
class HTMLValidator {
    ...
    checkAccessibility(html) {
        ...
        if (altMissing > 0) {
            this.fixes.push(`${altMissing}  alt=""  `);
        }

        //
        if (!html.match(/<html[^>]*lang=["'][^"']+["'][^>]*>/i)) {
            this.errors.push('html  lang  ');
            this.fixes.push('lang="ko" ');
        }
